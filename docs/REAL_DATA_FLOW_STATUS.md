...
| Add Visit | `public.visits`, `public.cafes`, required photos, signed-in `visit_photos` | Real Supabase-backed for signed-in users; local-only for signed-out users | `AddTabView`, `VisitService`, `CafeService`, `VisitPhotoUploadService`; fresh smoke visit `587f8423-a56f-46fe-b15a-452b2f024ebf` | Add clearer per-photo progress/errors |
...
- Add Visit UI: form shows photo-required copy and a 5-step progress state including Photo.
