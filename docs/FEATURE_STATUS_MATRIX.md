...
| Friends and profiles | People discovery source implemented; legacy social/profile contract production-configured | The new People hub adds improved search, requests/friends/sent states, selected-contact matching, profile links/QR, durable invitation recovery, mutual/shared-context suggestions, search dead ends, first-week prompting, and versioned discoverability preferences. Private identifiers remain in sealed tables and selected contacts are transient. Editorial Atlas profiles, blocks, and the existing friendship graph remain authoritative | Deploy the additive migration and Edge Functions to disposable QA, configure secrets/routes, run cross-account runtime acceptance, then production rollout and replacement-TestFlight acceptance |
