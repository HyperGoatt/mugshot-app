...
and the signed candidate is installed on the owner's iPhone. Owner hands-on and
...
| Native Home and Recipes | Production-configured; enabled candidate installed | Unified Coffee/Component/Drink/Blank recipes, two-surface logging, guided preparation, resumable batches, linked immutable versions, repeat/comparison, adaptation, private sync/media, and explicit sharing. Production is at migration 177; pre/post content and Storage evidence matched; 22 activation-focused tests pass; the signed Debug app is installed on Joe's iPhone | Owner hands-on acceptance; TestFlight remains a separate explicit gate |
