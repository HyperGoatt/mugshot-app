...
| Remote push | V2 production configured; v3 locally verified; physical acceptance pending | APNs worker, credentials, topics, queue, compatible v2/v3 device and delivery RPCs, opt-in badge payload | QA/live v3 release, then sandbox/production device lifecycle matrix |
...
1. Release the backward-compatible badge and scheduler contracts through QA.
