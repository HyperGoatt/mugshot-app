...
| Friends and profiles | People discovery production-configured; signed app relaunched | The new People hub adds improved search, requests/friends/sent states, selected-contact matching, profile links/QR, durable invitation recovery, mutual/shared-context suggestions, search dead ends, first-week prompting, and versioned discoverability preferences. Private identifiers remain in sealed tables and selected contacts are transient. All 66 hosted SQL contracts and the real Auth/Edge harness passed; production is aligned at 179 migrations, both People functions are active at version 3, all four reversible capabilities are enabled, and the paid QA branch is deleted. Editorial Atlas profiles, blocks, and the existing friendship graph remain authoritative | Hands-on People-flow acceptance on the connected iPhone and replacement-TestFlight acceptance |
