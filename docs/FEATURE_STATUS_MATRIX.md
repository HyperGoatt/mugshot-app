...
truthful queued-place Share receipt, main-app import status and account fence,
actual in-app share handoff copy, People retry, and iPhone-only target settings
are in source. They still require exact-candidate acceptance. A
transient Home toolbar screenshot did not reproduce after settling; full Home
acceptance remains open. The distributed TestFlight 0.5.3 (8) does not
...
unchanged; the shared production head is now repository-aligned at migration 182.
...
| Account export/deletion | Backend trust gate accepted on data-less QA; production at 182 migrations | Export manifest, step-up deletion, direct-worker cleanup, private media removal and repeat acknowledgement passed hosted; receipt timestamp repair deployed with preservation checks | Apple provider revocation and signed-client acceptance remain separate |
