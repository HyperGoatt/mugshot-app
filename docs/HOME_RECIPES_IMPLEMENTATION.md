...
1. Unified inline Journal Home navigation, per-tab scroll/filter restoration,
   and legacy recipe adoption into the new collection.
2. Full built-in field visibility/label/order customization, method-specific
   actuals refinement, and nested component preparation without duplicate logs.
...
6. Method-relevant attempt comparison with bean/equipment context, complete
