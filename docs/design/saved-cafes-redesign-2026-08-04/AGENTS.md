...
### Locked Saved cafes direction

- The source of visual truth is `concepts/00-locked-combined-direction.png`.
- Preserve Option 1's journal-first native bottom-sheet structure and Option 2's compact horizontal search bar across the full board set.
- The medium sheet keeps this order: cafe identity, full-width `Log a Sip`, four labeled state actions, then `Your Mugshot`.
- Saved remains list-based. The existing Map tab is the sole map surface and keeps its Favorites, Want to Try, Visited, Friends, and discovery filters.
- The locked map composition represents the shared cafe-detail sheet opened from the existing Map tab, not a second Map implementation inside Saved.
- Comfortable cards are the default, compact rows are supported, and Lists stay visually unchanged except for the cafe-membership entry sheet.
- Comfortable cards expose one-tap Favorite, Want to Try, and Log a Sip. Compact rows keep Favorite and Want to Try visible and move Log a Sip into overflow.
- `Show on Map` lives in card overflow and expanded detail secondary actions; it opens the existing Map tab centered on the selected cafe.
- Map scope options are All, Visited, Favorites, Want to Try, Friends, and Discovery; All is the default.
- Authorized Mugshot media becomes the cafe cover. Future owner media is deferred; external place-photo providers and open-license ingestion are out of scope. All other cafes use a Mugshot-branded no-photo treatment.
- A first sip at a Want to Try cafe triggers a brief Mugsy celebration, clears Want to Try, and offers Undo with a Reduce Motion fade fallback.
- The approved package is 36 screens, six component boards, and six specification boards. Produce four checkpoint mockups and pause before the remaining exports.
- Deliver deterministic light-appearance PNG boards at the dimensions defined in the manifest. Do not use generated UI text or introduce production app changes.
