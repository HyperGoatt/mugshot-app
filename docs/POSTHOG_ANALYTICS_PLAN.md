...
| People discovery | `people_hub_opened`, `people_profile_share_opened`, `people_search_completed`, `people_friend_request_completed`, `people_suggestion_opened`, `people_suggestion_dismissed`, `people_contacts_started`, `people_contacts_selection_completed`, `people_invite_created`, `people_invite_handoff_completed`, `people_invite_resolved`, `people_first_week_prompt` | allowlisted source/action/outcome/reason (`people_v2` included)/mode=`invite`, ranking version, bounded duration and coarse count buckets only; never queries, names, phone numbers, contacts, item keys, social IDs, codes, tokens, or raw errors |
