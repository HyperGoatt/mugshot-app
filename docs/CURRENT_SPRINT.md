...
| Physical sandbox acceptance | Blocked at profile generation | Push Notifications is enabled for `co.mugshot.app.dev`; the replacement profile is staged with the existing development certificate and registered iPhone but is not yet generated/downloaded/installed |
...
| Signed physical Debug | `co.mugshot.app.dev`, development entitlement, sandbox environment, v3 badge registration | Generate, download, and install the staged development profile, then run APNs registration, foreground/background/terminated delivery, routes, badges, preferences, and sign-out |
