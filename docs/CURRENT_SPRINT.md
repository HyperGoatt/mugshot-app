...
| TestFlight production acceptance | 0.5.3 (6) upload authorized; release gates in progress | Pass the exact candidate on Simulator and connected iPhone, archive/upload, wait for processing, then assign Alpha Friends with notes exactly `Welcome to Mugshot!` |
| Revised 44-report remediation | Implemented and locally verified; physical app launch passed, feature acceptance pending | Five workstreams are represented on `codex/testflight-feedback-remediation`; 425 unit tests, eight focused UI journeys, the 43-screenshot plus one-text review, and full-static 12/0/1 passed. The latest Feed-motion source built, installed, and launched as `co.mugshot.app.dev` on Joe's connected iPhone. No report will be resolved until its behavior is manually accepted in the replacement TestFlight build |
| Simulator-QA follow-up | Sixth source batch locally verified; fourth batch physically launched | Feed reselect/top behavior plus scope pills with equal eight-point upper/lower resting gaps that ignore refresh pull, hold for 60 upward points, and slide continuously beneath the header over their measured height; Journal toolbar flag and Taste Passport holding screen; all-step composer close; explicit multi-draft recovery; 12–18 preparation-specific stationary criteria; persistent importance; compact Publish controls; private-Storage cafe photos; and non-destructive multi-ID cafe stitching are in source. The latest Debug build/launch, focused motion/gap coverage, live Feed capture, and comparison board pass in Simulator. The preceding source installed and launched on Joe's iPhone; this equal-spacing follow-up has not been promoted |
| Editorial Atlas profile | Latest Profile-share media follow-up locally verified; prior source physically launched; profile contract production-configured | The approved foam-white statistics dock and four tabs remain, the banner is back at 112 points, Favorite Spots is a compact text rail, and Add Favorite Spot chooses the reason before the cafe. The default-on Friends-on-profile setting publishes Friends plus Everyone profile content while keeping Private absent; turning it off restores Everyone-only profile content. Share Profile renders fixed Story/Post snapshots, sorts the current profile-published Mugshots newest-first, resolves durable private-Storage media before rendering, and hands iOS the artwork, `Add me on Mugshot` copy, and canonical active link. Focused Swift tests, live Simulator share render, and profile/share comparison pass. Migration `20260826143102` is live and its expected tables/RPCs resolve; the latest media fix has not been promoted to physical or TestFlight acceptance |
...
| TestFlight | 0.5.3 (6), `co.mugshot.app`, production entitlement/environment, v3 badge registration | Simulator and connected-iPhone candidate gates, authorized upload, processing, Alpha Friends assignment, production matrix |
