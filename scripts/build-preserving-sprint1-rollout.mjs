...
assert.ok(destination, 'Usage: node scripts/build-preserving-sprint1-rollout.mjs output.sql');
...
const files = fs.readdirSync('supabase/migrations').filter(f =>
  f.endsWith('.sql') && (f.slice(0, 14) > '20260826143102' || f.startsWith('20260825030917'))
).sort();
assert.equal(files.length, 37, 'Re-review the rollout when migration scope changes');
const sql = [`begin;
set local lock_timeout='5s';
set local statement_timeout='120s';
do $$ begin
 if (select count(*) from supabase_migrations.schema_migrations)<>127
 or (select max(version) from supabase_migrations.schema_migrations)<>'20260826143102'
...
sql.push(`
-- Staged owner rollout: preserve historical notification copy and the original
-- bucket visibility. Never make an originally Private bucket public. New media
-- still uses authorized signing; global bucket protection awaits client adoption.
update public.activity_events e set title=o.title,body=o.body,metadata=o.metadata
from rollout_original_activity o where e.id=o.id and (e.title,e.body,e.metadata) is distinct from (o.title,o.body,o.metadata);
update storage.buckets b set public=o.public from rollout_original_buckets o where b.id=o.id and b.public is distinct from o.public;
select public.configure_screening_schedule_v1(false);
do $$
