...
begin
 ...
 if exists(select 1 from public.get_visit_v3_reflection_v1(test_visit)) then raise exception 'rejected reflection exposed';end if;
 update private.screening_jobs set state='approved',reason='human_review'
 where subject_kind='visit' and subject_id=test_visit;
 insert into public.comments(id,user_id,visit_id,text) values(parent_id,owner_id,test_visit,'Synthetic safe comment');
 if not exists(select 1 from public.list_visit_comments_v2(test_visit) where id=parent_id) then raise exception 'locally admitted comment unavailable';end if;
 insert into public.comments(id,user_id,visit_id,text,parent_comment_id) values(reply_id,owner_id,test_visit,'Synthetic safe reply',parent_id);
 ...
 if not exists(select 1 from public.list_visit_comments_v2(test_visit) where id=parent_id and text='Synthetic edited comment') then raise exception 'safe edited comment was not locally admitted';end if;
 ...
 update private.screening_jobs set state='rejected',reason='human_review'
 where subject_kind='recipe' and subject_id=version_id;
 if exists(select 1 from public.get_recipe_identity_for_visit_v1(test_visit)) then raise exception 'rejected recipe identity exposed';end if;
 update private.screening_jobs set state='approved',reason='human_review'
 where subject_kind='recipe' and subject_id=version_id;
 perform set_config('request.jwt.claim.sub',owner_id::text,true);
