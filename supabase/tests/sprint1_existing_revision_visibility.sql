...
begin
 ...
 if private.screening_approved_v1('visit',item) then raise exception 'new pending content visible';end if;
 -- Simulate the migration snapshot as database owner, never a client API.
 ...
 if not private.screening_approved_v1('visit',item) then raise exception 'original shared revision disappeared';end if;
 update private.screening_jobs set state='needs_review' where subject_kind='visit' and subject_id=item;
 ...
 if private.screening_approved_v1('visit',item) or exists(select 1 from private.screening_existing_visibility where subject_id=item) then raise exception 'edit retained old eligibility';end if;
 insert into private.screening_existing_visibility(subject_kind,subject_id,revision)
 select subject_kind,subject_id,revision from private.screening_jobs where subject_kind='visit' and subject_id=item;
 update private.screening_jobs set state='rejected' where subject_kind='visit' and subject_id=item;
 update private.screening_jobs set state='needs_review' where subject_kind='visit' and subject_id=item;
 if private.screening_approved_v1('visit',item) then raise exception 'appeal restored rejected eligibility';end if;
 update private.screening_jobs set state='approved' where subject_kind='visit' and subject_id=item;
