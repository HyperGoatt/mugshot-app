-- Owner-requested exports are assembled server-side so the caller cannot
-- substitute another user ID. The JSON preserves private notes as a separate
-- collection instead of merging them into shareable visit rows.
...
begin
  ...
  return jsonb_build_object(
    ...
    'journal_entries', coalesce((
      ...
      from public.visits visit where visit.user_id = actor
    ), '[]'::jsonb),
    'private_notes', coalesce((
      select jsonb_agg(to_jsonb(note) order by note.created_at, note.visit_id)
      from public.visit_private_notes note where note.user_id = actor
    ), '[]'::jsonb),
    'visit_photos', coalesce((
      select jsonb_agg(to_jsonb(photo) order by photo.created_at, photo.sort_order)
    ...
    'drink_analyses', coalesce((
      ...
      from public.visit_drink_analyses analysis where analysis.user_id = actor
    ), '[]'::jsonb),
    'journal_bookmarks', coalesce((
      select jsonb_agg(to_jsonb(bookmark) order by bookmark.created_at, bookmark.visit_id)
      from public.visit_bookmarks bookmark where bookmark.user_id = actor
    ), '[]'::jsonb),
    'recipe_identities', coalesce((
      select jsonb_agg(
        to_jsonb(identity) || jsonb_build_object(
          'versions', coalesce((
            select jsonb_agg(to_jsonb(version) order by version.version_number)
            from public.recipe_versions version where version.recipe_identity_id = identity.id
          ), '[]'::jsonb)
        ) order by identity.created_at, identity.id
      ) from public.recipe_identities identity where identity.user_id = actor
    ), '[]'::jsonb),
    'taste_signals', coalesce((
      select jsonb_agg(to_jsonb(signal) order by signal.updated_at, signal.id)
      from public.taste_signals signal where signal.user_id = actor
    ), '[]'::jsonb),
    'cafe_lists', coalesce((
      select jsonb_agg(
        to_jsonb(listing) || jsonb_build_object(
          'items', coalesce((
            ...
            from public.cafe_list_items item where item.list_id = listing.id
          ), '[]'::jsonb),
          'members', coalesce((
            select jsonb_agg(to_jsonb(member) order by member.created_at, member.user_id)
            from public.cafe_list_members member where member.list_id = listing.id
          ), '[]'::jsonb)
        ) order by listing.created_at, listing.id
      ) from public.cafe_lists listing where listing.owner_id = actor
    ), '[]'::jsonb),
    ...
    'capture_preferences', coalesce((
      select to_jsonb(preference) from public.user_capture_preferences preference where preference.user_id = actor
    ), '{}'::jsonb),
    'reflection_preferences', coalesce((
      select to_jsonb(preference) from public.user_reflection_preferences preference where preference.user_id = actor
    ), '{}'::jsonb),
    'media_references', coalesce((
      select jsonb_agg(distinct media.url)
      from (
        select photo.photo_url as url
        from public.visit_photos photo
        join public.visits visit on visit.id = photo.visit_id
        where visit.user_id = actor
        union all
        select profile.avatar_url from public.users profile where profile.id = actor and profile.avatar_url is not null
        union all
        select profile.banner_url from public.users profile where profile.id = actor and profile.banner_url is not null
      ) media where media.url is not null and btrim(media.url) <> ''
    ), '[]'::jsonb)
  );
end;
$$;
