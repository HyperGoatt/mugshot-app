...
begin
  ...
  return jsonb_build_object(
    ...
    'media_references',coalesce((select jsonb_agg(distinct media.url) from (
      select photo.photo_url as url from public.visit_photos photo join public.visits visit on visit.id=photo.visit_id where visit.user_id=actor
      union all select profile.avatar_url from public.users profile where profile.id=actor and profile.avatar_url is not null
      union all select profile.banner_url from public.users profile where profile.id=actor and profile.banner_url is not null
    ) media where media.url is not null and btrim(media.url)<>''),'[]'::jsonb)
  );
end; $$;
